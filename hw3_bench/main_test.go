...
//-----------
	go tool pprof main.test.exe cpu.out  -- вызывает pprof
list FastSearch
	go tool pprof main.test.exe mem.out

	go tool pprof -svg -inuse_space hw3_bench.test.exe mem.out > mem_is.svg
	go tool pprof -svg -inuse_objects hw3_bench.test.exe mem.out > mem_io.svg
	go tool pprof -svg hw3_bench.test.exe cpu.out > cpu.svg
*/
